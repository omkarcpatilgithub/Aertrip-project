...
class Parser:
    ...
    def load_models(self, loc=None, allow_pickle=False):
        if not loc:
            loc = os.path.join(os.path.expanduser('~'), '.ner_model')
        self.model = load_model(os.path.join(loc,"model.h5"))
        # loading word2Idx
        self.word2Idx = np.load(os.path.join(loc,"word2Idx.npy"),allow_pickle=allow_pickle).item()
        # loading idx2Label
        self.idx2Label = np.load(os.path.join(loc,"idx2Label.npy"),allow_pickle=allow_pickle).item()
    ...
    def learn_predict(self,Sentence):
        Sentence = words = word_tokenize(Sentence)
        print("1st")
        print(Sentence)
        Sentence = self.addCharInformation(Sentence)
